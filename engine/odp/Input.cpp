...
void Input::notify() {
    auto observers = events[event.type].subEvents.equal_range(events[event.type].findIndex());
    for (auto iter = observers.first; iter != observers.second; iter++) {
        asyncCalls.push_back(std::async(std::launch::async, &Observer::pushEvent, iter->second, EventAlert({event, {event.type, events[event.type].findIndex()}})));
    }
}
...
void Input::execute() {
    ...
    for (auto& eventWrapper: events) {
        for (auto& observer: eventWrapper.second.subEvents) {
            asyncCalls.push_back(std::async(std::launch::async, &Observer::update, observer.second));
        }
    }
    for (auto& asyncCall: asyncCalls) {
        asyncCall.get();
