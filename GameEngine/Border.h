...
class Border {
public:
    Border(const Vector2D& prevVertex, const Vector2D& nextVertex);
    
    std::pair<std::pair<bool, Vector2D>, std::pair<bool, bool>> checkBordersIntersection(const Border& border, const Vector2D& referencePosition = Vector2D(
            0, 0), const Vector2D& externalPosition = Vector2D(0, 0));
