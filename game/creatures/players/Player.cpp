...
Player::Player(const std::string& texturePath, int speed, unsigned short hp, unsigned short mana, unsigned short accuracy, double critRate, std::function<void(Entity*)> specialAbility, std::function<void(Entity*)> passiveAbility) : Entity(texturePath, true), hp(hp), mana(mana), accuracy(accuracy), critRate(critRate), specialAbility(specialAbility), passiveAbility(passiveAbility) {
