...
TEST_F(EntityFixture, resolveCollision) {
    ...
    for (int i = 0; i < 4; i++) {
        std::cout << "(" << std::next(entity.getCollider("1").getBordersBegin(), i)->getX() << ", " << std::next(
                entity.getCollider("1").getBordersBegin())->getY() << ") ";
    }
    std::cout << std::endl;
    std::cout << "External: (" << external.getTransform().getPosition().getX() << ", " << external.getTransform().getPosition().getY() << ") borders: ";
    for (int i = 0; i < 4; i++) {
        std::cout << "(" << std::next(external.getCollider("2").getBordersBegin(), i)->getX() << ", " << std::next(
                external.getCollider("2").getBordersBegin())->getY() << ") ";
    }
    std::cout << std::endl;
